...
async function openBookingModal(movieId) {
  ...
  const shows = await fetch(`/DuAnWeb/api/get_showtimes.php?movieId=${movieId}`).then(r => r.json());
...
async function loadShowtimes(movieId, date) {
  ...
  try {
    const res = await fetch(`/DuAnWeb/api/get_showtimes.php?movieId=${movieId}&date=${date}`);
...
//   fetch(`/DuAnWeb/api/get_booked_seats.php?showtime_id=${id}`)
//     .then(r => r.json())
//     .then(booked => {
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    items.forEach((product) => {
      ...
      productElement.innerHTML = `
                ...
                <div class="product-info">
                    ...
                    <a href="/Home/ChiTietSanPham.jsp?movieId=${product.id}" class="btn-secondary">Xem Chi Tiết</a>
                </div>
            `;
  ...
  async function fetchMovies() {
    try {
      const res = await fetch('/DuAnWeb/api/movies.php');
      ...
      list.forEach(m => {
        ...
        products.push({
          ...
          image: m.PosterUrl || 'api/placeholder.php?width=300',
  ...
  if (productDetail && productDescription && productNutrition && relatedProducts) {
    ...
    if (!product) {
      ...
      <div class="product-not-found">
        ...
        <a href="/Home/ChiTietSanPham.jsp" class="btn">Quay lại</a>
      </div>`;
  ...
  `;
    ...
    related.forEach(rp => {
      ...
      <div class="product-info">
        ...
        <a href="/Home/ChiTietSanPham.jsp?movieId=${rp.id}" class="btn-secondary">Xem Chi Tiết</a>
      </div>`;
...
if (adminBtn && adminModal) {
  ...
  loginBtn.addEventListener('click', () => {
    ...
    window.location.href = '/duanweb/Admin/QuanLyPhim.jsp';
  });
}
...
//     const movieId = new URLSearchParams(window.location.search).get('movieId');
//     const allShows = await fetch(`/DuAnWeb/api/showtimes.php?movieId=${movieId}`).then(r => r.json());
...
if (form) {
  ...
  form.addEventListener('submit', async e => {
    ...
    // 2) Gửi JSON đúng key mà book.php đọc (showtimeId)
    try {
      const res = await fetch('/DuAnWeb/api/book.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ showtimeId, seats, customer, phone })
      });
      if (!res.ok) {
        const text = await res.text();
        console.error('Lỗi HTML từ server:', text);
        return alert(`Đặt vé thất bại (mã ${res.status})`);
      }
      ...
      // alert('Đặt vé thành công! Mã nhóm = ' + data.group);
      const count = data.seats.length;
      if (count === 1) {
        alert('Đặt vé thành công! Mã Booking = ' + data.bookingId);
        window.open(`/DuAnWeb/HoaDon.jsp?bookingId=${data.bookingId}`, '_blank');
      } else {
        alert('Đặt vé thành công! Mã nhóm = ' + data.group);
        window.open(`/DuAnWeb/HoaDon.jsp?group=${data.group}`, '_blank');
      }
      document.getElementById('booking-form').reset();
      if (data.group) {
        // mở tab mới hiển thị hoá đơn
        const invoiceUrl = `HoaDon.jsp?group=${data.group}`;
        window.open(invoiceUrl, '_blank');
        window.location.href = '/History/LichSuDatVeNguoiDung.jsp';
      } else {
        alert('Đặt vé thất bại, thử lại.');
      }
...
async function loadSeatMap(showtimeId) {
  ...
  const booked = await fetch(`/DuAnWeb/api/get_booked_seats.php?showtime_id=${showtimeId}`).then(r => r.json());
...
async function loadMovieDetails(movieId) {
  const res = await fetch(`/DuAnWeb/api/movies.php?movieId=${movieId}`);
  ...
  if (vid) {
    vid.src = movie.Trailer_ID || 'api/placeholder.php?width=1200&height=800';
    vid.poster = movie.PosterUrl || 'api/placeholder.php?width=1200&height=800';
  }
}
...
function attachBookingEvents() {
  ...
  bookBtn.addEventListener('click', async () => {
    ...
    const shows = await fetch(`/DuAnWeb/api/showtimes.php?movieId=${movieId}`)
      .then(r => r.json());
    ...
    const poster = currentMovie.PosterUrl || currentMovie.image || 'api/placeholder.php';
